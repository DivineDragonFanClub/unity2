...
fn resolve_offset_with_host_override(name: &[u8], pattern: &str, label: &str) -> usize {
    debug_assert_eq!(name.last(), Some(&0u8), "name must be NUL-terminated");
    let mut addr: usize = 0;
    let _ = unsafe { skyline::nn::ro::LookupSymbol(&mut addr as *mut usize, name.as_ptr()) };
    if addr != 0 {
...
const CLASS_FROM_NAME_PATTERN: &str =
...
pub(crate) unsafe fn class_from_name(
    image: &Il2CppImage,
    namespace: *const u8,
    name: *const u8,
    ...
    let fn_addr = *CACHED.get_or_init(|| {
        let off = resolve_offset_with_host_override(
            b"unity2_override_il2cpp_class_from_name\0",
            CLASS_FROM_NAME_PATTERN,
            "il2cpp_class_from_name",
        );
        let text_base = skyline::hooks::getRegionAddress(skyline::hooks::Region::Text) as usize;
        text_base + off
    });
    let f: Fn = core::mem::transmute(fn_addr);
...
const CLASS_FROM_IL2CPPTYPE_PATTERN: &str =
    "ff 03 01 d1 fd 7b 01 a9 fd 43 00 91 f6 57 02 a9 f4 4f 03 a9 f3 03 00 aa e0 03 1f aa 68 2a 40 39 08 05 00 51 1f 75 00 71";

pub unsafe fn class_from_il2cpptype(ty: &Il2CppType) -> Option<&'static mut Il2CppClass> {
    type Fn = unsafe extern "C" fn(&Il2CppType) -> Option<&'static mut Il2CppClass>;
    static CACHED: OnceLock<usize> = OnceLock::new();
    let fn_addr = *CACHED.get_or_init(|| {
        let off = resolve_offset_with_host_override(
            b"unity2_override_il2cpp_class_from_il2cpptype\0",
            CLASS_FROM_IL2CPPTYPE_PATTERN,
            "il2cpp_class_from_il2cpptype",
        );
        let text_base = skyline::hooks::getRegionAddress(skyline::hooks::Region::Text) as usize;
        text_base + off
    });
