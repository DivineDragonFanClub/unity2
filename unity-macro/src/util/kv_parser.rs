...
impl KvParser {
    ...
    /// a `venial::Attribute`.
    pub fn parse_args(attr_name: &str, args: TokenStream, span: Span) -> ParseResult<Self> {
        let tokens: Vec<TokenTree> = args.into_iter().collect();
        let map = ParserState::parse_token_slice(attr_name.to_string(), &tokens)?;
        Ok(Self { map, span })
    }
...
impl<'a> ParserState<'a> {
    ...
    pub fn parse_token_slice(attr_name: String, tokens: &'a [TokenTree]) -> ParseResult<KvMap> {
